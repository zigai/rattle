...
Rattle can be used to lint as you type as well as to format files.
...
the Rattle LSP server. See the :ref:`lsp command <lsp_command>` for command
usage details.
...
- VSCode:
  ...
  - `Rattle (Unofficial) <https://marketplace.visualstudio.com/items?itemName=llllvvuu.fixit-unofficial>`_
    (via VSCode Marketplace; compiled from Generic LSP Client with preset
    configuration for Rattle)
...
Rattle can be included as a hook for `pre-commit <https://pre-commit.com>`_.

Once you `install it <https://pre-commit.com/#installation>`_, you can add
Rattle's pre-commit hook to the ``.pre-commit-config.yaml`` file in
your repository.
...
That way VSCode opens the editor at the right position when clicking on code locations in Rattle's terminal output.
