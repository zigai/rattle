...
Rattle uses `TOML format <https://toml.io>`_ for configuration, and supports
hierarchical, cascading configuration. Rattle will read values from both the
...
When determining the configuration to use for a given path, Rattle will continue
...
Rattle will then read configuration values from each file, from further to
...
.. attribute:: root
    ...
    If set to ``true``, Rattle will not visit any configuration files further up
...
.. attribute:: enable
    ...
    Rattle will enable the built-in :mod:`fixit.rules` lint rules by default.
...
.. attribute:: output-format
    ...
    - ``custom``: Specify your own format using the :attr:`output-template`
      option below.
    - ``fixit``: Rattle's default human-readable format, with Ruff-style headers,
