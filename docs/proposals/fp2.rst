...
By default, Rattle expects rules to be referenced by their fully-qualified
module and/or class name. For example, ``fixit.rules`` refers to the bundled set
if lint rules shipped with Rattle. Third party rules available in the environment
can similarly be referenced by their module name, as long as they are installed
and importable by Rattle at runtime. These will be collectively referred to as
"global rules" here for sake of clarity.

However, in many cases, it is also useful for projects using Rattle to build
...
first install those lint rules into the environment before running Rattle.

These are defined as "local rules", and will require dedicated behavior from
Rattle to discover and import these rules at runtime, as well as special
...
When gathering enabled/disabled rules from configuration, Rattle currently
